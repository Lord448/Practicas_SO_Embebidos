...
/* Definitions for vTaskComm */
osThreadId_t vTaskCommHandle;
const osThreadAttr_t vTaskComm_attributes = {
  .name = "vTaskComm",
  .stack_size = 128 * 4,
...
int main(void)
{
  ...
  /* creation of vTaskComm */
  vTaskCommHandle = osThreadNew(StartTaskComm, NULL, &vTaskComm_attributes);
...
/**
* @brief Function implementing the vTaskComm thread.
* @param argument: Not used
* @retval None
*/
...
void StartTaskADC(void *argument)
{
  ...
  for(;;)
  {
    ...
    HAL_ADC_Stop(&hadc1);
    osMessageQueuePut(xFIFOAdcControlHandle, &ADC_Res, 0, 10); //Send to the Control Task
    osMessageQueuePut(xFIFOAdcUartHandle, &ADC_Res, 0, 10); //Send to the Comm Task
    osDelay(1);
  }
